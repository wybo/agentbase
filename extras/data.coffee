...
ABM.DataSet = class DataSet
  ...
  toDrawing: ->
    ABM.patches.installDrawing(img = @toImage())
    img

  toPatchColors: ->
    ABM.patches.installColors(img = @toImage())
    img
  ...
  normalize: (lo, hi) ->
    new DataSet @width, @height, u.array.normalize @data, lo, hi

  # Normalize a dataset to clamped Uint8 bytes [0-255]
  normalize8: ->
    new DataSet @width, @height, u.array.normalize8 @data
  ...
  filter: (f) ->
    new DataSet @width, @height, (f(d) for d in @data)
...
ABM.PatchDataSet = class PatchDataSet extends DataSet
  constructor: (f, arrayType = Array) ->
    ...
    @useNearest = true
  toPatchVar: (name) ->
    p[name] = @data[i] for p, i in ABM.patches
    null
