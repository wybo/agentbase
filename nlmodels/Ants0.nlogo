globals     [ nest-color food-color ]
turtles-own [ carrying-food? pheromone ]
patches-own [ food-pheromone nest-pheromone nest? food? ]
...
to setup-patches  ;; Initialize patch variables: Create nest and food areas
  ...
  ask patch-rect (max-pxcor - 6) 0 3 3 [set nest? true set pcolor nest-color]
  ask patch-rect (min-pxcor + 6) 0 3 3 [set food? true set pcolor food-color]
end
to setup-turtles ;; Initialize turtles: Position turtles on nest with "bug" shape
  create-turtles population
  [ set shape "bug" set size 3
    setxy (max-pxcor - 6) 0 ;; init turtles in middle of nest
    reset-turtle false ]
end
to reset-turtle [with-food?] ;; Reset turtle variables according to whether has food.
...
to update-turtles
  ask turtles
  [ if who >= ticks [ stop ] ;; gradually leave nest
    wiggle-uphill
    drop-pheromone ]
end
to wiggle-uphill ;; turtle procedure: Follow appropriate pheromone with a bit of wiggle.
  ifelse count neighbors4 != 4 [rt 180] ;; at edge, turn around.
  [ let p max-one-of (neighbors in-cone 2 180) [target-pheromone]
    if [target-pheromone] of p > (.001 / max-pheromone) [face p] ]
  rt (wiggle-angle / 2) - random wiggle-angle
  forward 1
end
to-report target-pheromone ;; patch procedure called by turtle: Report appropriate pheromone
  report ifelse-value [carrying-food?] of myself [nest-pheromone] [food-pheromone]
end
to drop-pheromone ;; turtle procedure: Drop appropriate pheromone
  if (not carrying-food? and food?) or (carrying-food? and nest?) [reset-turtle not carrying-food?]
  ifelse carrying-food?
  [ set food-pheromone food-pheromone + 0.1 * pheromone ]
...
to update-patches ;; Diffuse and evaporate pheromones, update patch colors
  ...
  ask patches
  [ set food-pheromone food-pheromone * (1 - evaporation-rate)
    set nest-pheromone nest-pheromone * (1 - evaporation-rate)
    if count neighbors4 != 4 [set food-pheromone 0 set nest-pheromone 0] ; set edges to no pheromone
    if not nest? and not food?
...
185
192
While running you can change all sliders below.
11
0.0
0
