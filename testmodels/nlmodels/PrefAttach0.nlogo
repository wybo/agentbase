;; A shorter version of the Model Library's Preferential Attachment example.
to setup
  clear-all
  set-default-shape turtles "circle"
  ;; make the initial network of two turtles and an edge
  crt 1  make-node turtle 0
  reset-ticks
end
to go
  tick
...
to make-node [to-node] ;; Create a new node connecting to to-node
  create-turtles 1
  [ create-link-with to-node
    move-to to-node  forward 8 ] ;; position the new node near its partner
end
to update-turtles
  ...
  [ layout-spring turtles links .5 1 .5 ];spring-constant spring-length repulsion-constant
end

;; Choose random node from a list where each node occurs the number of links it has.
;; Thus the greater number of links a node has, the more likely it is to be chosen.
to-report find-partner0
  let partners []
  ask turtles [repeat (count link-neighbors) [set partners lput self partners]]
  report one-of partners
end
to-report find-partner
  let l one-of links
  ask l [set l one-of both-ends]
  report l
end
...
287
124
Note: layout can be changed while running.
11
0.0
0
...
This is a modified version of the Models Library preferential attachment model, for use in the SFI CSSS Modeling class.  The rest of the notes are directly from that model.
