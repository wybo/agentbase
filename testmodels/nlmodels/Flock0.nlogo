;; Modified version of Model Library's Flocking model.
to setup
  clear-all
  ask patches [set pcolor black + random-float 1.5]
  create-turtles population [ set size 1.5 setxy random-xcor random-ycor] ;  set label who
  reset-ticks
end
to go
  tick
...
to flock  ;; turtle procedure: Separate, Align, & Cohere w/ nearby turtles
  let flockmates other turtles in-radius vision
  if any? flockmates [
    let nearest-neighbor min-one-of flockmates [distance myself]
    ifelse distance nearest-neighbor < minimum-separation
    [ separate nearest-neighbor ]
    [ align flockmates cohere flockmates ]
  ]
  forward .5 ; move partial patch for smooth animation.
end
to separate [nearest-neighbor]  ;; avoid crowding neighbours
  turn-towards ([towards myself] of nearest-neighbor)
end
to align [flockmates]  ;; steer towards average heading of neighbours
  turn-towards (average-heading flockmates)
end
to cohere [flockmates]  ;; steer towards average position of neighbours
  turn-towards (average-heading-towards flockmates)
end
to turn-towards [new-heading] ;; turn towards the new heading but never by more than "max-turn"
  let turn subtract-headings new-heading heading
  if abs turn > max-turn [ set turn max-turn * (abs turn) / turn ]
  right turn
end

to-report average-heading [flockmates]  ;; Find average of headings of flockmates.
;; We can't just average the heading variables. For example, the average of 1 and 359 should be 0,
;; not 180.  So we have to use trigonometry. We use safe-atan for undefined atan, see Model Library.
  report safe-atan sum [sin heading] of flockmates
                   sum [cos heading] of flockmates
end
to-report average-heading-towards [flockmates]  ;; turtle procedure
;; "towards myself" gives us the heading from the other turtle to me, but we want the heading
;; from me to the other turtle, so we add 180.  See safe-atan above
  report safe-atan sum [sin (towards myself + 180)] of flockmates
                   sum [cos (towards myself + 180)] of flockmates
end
...
to update-plot plot flock-vector-size end
to-report flock-vector-size ;; report a value approaching 1 if all turtles aligned.
  report sqrt ((sum [sin heading] of turtles) ^ 2 +
               (sum [cos heading] of turtles) ^ 2) / population
end
@#$#@#$#@
GRAPHICS-WINDOW
250
...
247
125
Sliders below can be changed while running.
11
0.0
0
...
This is a modified version of the Models Library flocking model, for use in the SFI CSSS Modeling class. See: http://backspaces.net/wiki/NetLogo_Tutorial

The rest of the notes are directly from the model library's model, check it out!
...
@#$#@#$#@
<experiments>
  <experiment name="100Step" repetitions="1" runMetricsEveryStep="false">
    <setup>random-seed 1
setup</setup>
    <go>go</go>
    <timeLimit steps="100"/>
    <metric>population</metric>
    <metric>flock-vector-size</metric>
    <steppedValueSet variable="vision" first="2" step="2" last="8"/>
    <steppedValueSet variable="max-turn" first="2" step="2" last="8"/>
    <steppedValueSet variable="minimum-separation" first="0.5" step="0.5" last="2"/>
  </experiment>
  <experiment name="FlockVecSize" repetitions="1" runMetricsEveryStep="false">
    <setup>random-seed 1
setup</setup>
    <go>go</go>
    <timeLimit steps="2000"/>
    <exitCondition>flock-vector-size &gt; .7</exitCondition>
    <metric>population</metric>
    <metric>ticks</metric>
    <metric>flock-vector-size</metric>
    <steppedValueSet variable="vision" first="2" step="2" last="8"/>
