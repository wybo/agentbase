...
ABM.DataSet = class DataSet
  ...
  getXY: (x, y) ->
    @checkXY x, y; @data[@toIndex x, y]

  # Set the data and int x, y coord of data array
  setXY: (x, y, num) ->
    @checkXY x, y
    @data[@toIndex x, y] = num
  ...
  toString: (p = 2, sep = ", ")->
    ...
    data = if p < 0 then @data else u.array.toFixed @data, p
    for i in [0...@height]
  ...
  toContext: ->
    ...
    if @normalizeImage
      if @gray
        data = u.array.normalize8 @data
      else
        data = u.array.normalizeInt @data, 0, Math.pow(2, 24) - 1
    else
  ...
  toDrawing: (model = @model) ->
    model.patches.installDrawing(img = @toImage())
    img

  toPatchColors: (model = @model) ->
    model.patches.installColors(img = @toImage())
    img
  ...
  toPatchVar: (name, model = @model) ->
    if (ps = model.patches).length is @data.length
      p[name] = @data[i] for p,i in ps
    else
      p[name] = @patchSample p.x, p.y, model for p in ps
    null
  ...
  coordSample: (x, y, tlx, tly, w, h) -> #brx, bry) ->
    xs = (x - tlx) * (@width - 1) / w
    ys = (tly - y) * (@height - 1) / h # convert to sample space
    @sample xs, ys

  patchSample: (px, py, model = @model) ->
    world = model.world
    @coordSample px, py, world.minXcor, world.maxYcor, world.numX, world.numY
  
  # Normalize a dataset to linear interpolation: [min,max] -> [lo, hi], float
  normalize: (lo, hi) ->
    new DataSet @width, @height, u.array.normalize(@data, lo, hi), @model

  # Normalize a dataset to clamped Uint8 bytes [0-255]
  normalize8: ->
    new DataSet @width, @height, u.array.normalize8(@data), @model
  ...
  resample: (width, height) ->
    ...
    data = []
    xScale = (@width - 1) / (width - 1)
    yScale = (@height - 1) / (height - 1)
  ...
  convolve: (kernel, factor = 1) -> # Factory: return new convolved dataset
    ...
    for y in [y0...h] by 1
      for x in [x0...w] by 1
        ...
        array.push u.array.sum(multiplied) * factor
    new DataSet w - x0, h - y0, array, @model

  # A few common convolutions.  dzdx/y are also called horiz/vert Sobel
  dzdx: (n = 2, factor = 1 / 8) ->
    @convolve([-1, 0, 1, -n, 0, n, -1, 0, 1],factor)

  dzdy: (n = 2, factor = 1 / 8) ->
    @convolve([1, n, 1, 0, 0, 0, -1, -n, -1],factor)
  ...
  blur: (factor = 0.0625) ->
    @convolve([1, 2, 1, 2, 4, 2, 1, 2, 1], factor) # 1/16 = 0.0625

  edge: ->
    @convolve([1, 1, 1, 1, -7, 1, 1, 1, 1])

  # Return filtered dataset by applying f to each dataset element
  filter: (f) ->
    new DataSet @width, @height, (f(d) for d in @data), @model
...
ABM.PatchDataSet = class PatchDataSet extends DataSet
  ...
  toPatchVar: (name) ->
    p[name] = @data[i] for p, i in @model.patches
    null
