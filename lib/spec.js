(function() {
  ...
  ABM.test.Model = Model = (function(_super) {
    ...
    Model.prototype.setupAgents = function() {
      var agent, i, max, _i, _len, _ref, _results;
      i = this.world.min.x;
      max = this.world.max.x - this.world.min.x + 1;
      _ref = this.agents.create(max);
      _results = [];
  ...
  describe("Array", function() {
    ...
    describe("sample", function() {
      ...
      it("returns sample for which true if condition and number is given", function() {
        var array;
        u.randomSeed(2);
      ...
      it("returns an empty array if none are found", function() {
        var array;
        u.randomSeed(2);
        array = new ABM.Array(1, 2, 3, 4, 5, 6).sample(3, function(number) {
          return number > 10;
        });
      ...
      it("returns a sample without duplicates", function() {
        var array;
        u.randomSeed(2);
        array = new ABM.Array(1, 2, 2, 3, 4, 5, 6, 6, 5).sample(5, function(number) {
          return number > 1;
        });
        return expect(array).toEqual(new ABM.Array(5, 2, 6, 3, 4));
      });
      return it("returns a sample even if there are too few elements", function() {
        var array;
        u.randomSeed(2);
        array = new ABM.Array(1, 2, 2, 3, 4, 5, 6, 6, 5).sample(10, function(number) {
          return number > 1;
        });
        return expect(array).toEqual(new ABM.Array(5, 2, 6, 3, 4));
      });
  ...
  describe("Patch", function() {
    ...
    describe("distance", function() {
      return it("returns distance to the point", function() {
        var model, patch;
        model = t.setupModel();
        patch = model.patches.patch({
          x: 1,
