...
(function() {
  ...
  ABM.DataSet = DataSet = (function() {
    ...
    DataSet.prototype.toPatchVar = function(name) {
      ...
      return new PatchDataSet(ps.numX, ps.numY, data);
    };
    ...
    DataSet.prototype.slopeAndAspect = function(cellsize, noNaNs, posAngle) {
      var aspect, gx, gy, rad, slope, x, y, _i, _j, _ref, _ref1;
      if (cellsize == null) {
        cellsize = 1;
      }
      if (noNaNs == null) {
      ...
      if (this.slope == null) {
        this.dzdx = this.convolve([-1, 0, 1, -2, 0, 2, -1, 0, 1], 1 / 8);
        this.dzdy = this.convolve([1, 2, 1, 0, 0, 0, -1, -2, -1], 1 / 8);
        aspect = [];
        slope = [];
        for (y = _i = 0, _ref = this.height; _i < _ref; y = _i += 1) {
          for (x = _j = 0, _ref1 = this.width; _j < _ref1; x = _j += 1) {
            gx = this.dzdx.getXY(x, y);
            gy = this.dzdy.getXY(x, y);
            slope.push(Math.atan(Math.sqrt(gx * gx + gy * gy) / cellsize));
            while (noNaNs && gx === gy) {
              gx += u.randomNormal(0, .01);
              gy += u.randomNormal(0, .01);
            }
        ...
        this.slope = new DataSet(this.width, this.height, slope);
        this.aspect = new DataSet(this.width, this.height, aspect);
      }
      return [this.slope, this.aspect, this.dzdx, this.dzdy];
    };
  ...
  ABM.ImageDataSet = ImageDataSet = (function(_super) {
    ...
    ImageDataSet.imageRowsToData = function(img, rowsPerSlice, arrayType, f) {
  ...
  ABM.PatchDataSet = PatchDataSet = (function(_super) {
    ...
    function PatchDataSet(f, height, data) {
      var p, ps;
      if (u.isArray(data)) {
        PatchDataSet.__super__.constructor.call(this, f, height, data);
      } else {
        if (u.isString(f)) {
          f = u.propFcn(f);
        }
        PatchDataSet.__super__.constructor.call(this, (ps = ABM.patches).numX, ps.numY, (function() {
    ...
    PatchDataSet.prototype.toPatchVar = function(name) {
      ...
      data = (function() {
        ...
        for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
          p = _ref[i];
          _results.push(p[name] = this.data[i]);
        }
        return _results;
      }).call(this);
      return new PatchDataSet(ps.numX, ps.numY, data);
