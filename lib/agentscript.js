...
(function() {
  ...
  ABM.util = u = {
    ...
    aToFixed: function(array, precision) {
    ...
    contains: function(array, object, propertyOrFunction) {
      return this.indexOf(array, object, propertyOrFunction) >= 0;
    },
    indexOf: function(array, object, propertyOrFunction) {
      if (propertyOrFunction == null) {
        propertyOrFunction = null;
      }
      if (propertyOrFunction != null) {
        return this.sortedIndex(array, object, propertyOrFunction);
      } else {
        return array.indexOf(object);
      }
    },
    sortedIndex: function(array, object, propertyOrFunction) {
      var call, high, low, mid, value;
      if (propertyOrFunction == null) {
        propertyOrFunction = function(o) {
          return o;
        };
      }
      if (this.isString(propertyOrFunction)) {
        call = this.propertyFunction(propertyOrFunction);
      } else {
    ...
    remove: function(array, object, propertyOrFunction) {
      var index;
      index = this.indexOf(array, object, propertyOrFunction);
      if (index !== -1) {
        return array.splice(i, 1);
      } else {
        return this.error("remove: object not found");
      }
    },
    removeItems: function(array, objects, propertyOrFunction) {
      var object, _i, _len;
      for (_i = 0, _len = objects.length; _i < _len; _i++) {
        object = objects[_i];
        this.remove(array, object, propertyOrFunction);
      }
      return array;
    },
    insert: function(array, object, propertyOrFunction) {
      var index;
      index = this.sortedIndex(array, object, propertyOrFunction);
      if (array[index] === object) {
        this.error("insert: object already in array");
      }
    ...
    fillCtx: function(ctx, color) {
      if (ctx.fillStyle != null) {
        this.setIdentity(ctx);
        ctx.fillStyle = this.colorString(color);
  ...
  ABM.BreedSet = (function(_super) {
    ...
    BreedSet.prototype.floodFill = function(aset, fCandidate, fJoin, fCallback, fNeighbors, asetLast) {
      ...
      if (fCallback != null) {
        fCallback(aset, asetNext);
      }
      if (asetNext.length > 0) {
        return this.floodFill(asetNext, fCandidate, fJoin, fCallback, fNeighbors, aset);
      }
    };
    ...
    BreedSet.prototype.maxProp = function(prop) {
      return u.aMax(this.getProp(prop));
    };

    BreedSet.prototype.minProp = function(prop) {
      return u.aMin(this.getProp(prop));
    };

    BreedSet.prototype.histOfProp = function(prop, bin) {
      if (bin == null) {
        bin = 1;
      }
    ...
    BreedSet.prototype.sortById = function() {
    ...
    BreedSet.prototype.minOneOf = function(f, valueToo) {
      if (valueToo == null) {
        valueToo = false;
      }
    ...
    BreedSet.prototype.maxOneOf = function(f, valueToo) {
      if (valueToo == null) {
        valueToo = false;
      }
    ...
    BreedSet.prototype.draw = function(ctx) {
      var o, _i, _len;
      u.clearCtx(ctx);
      for (_i = 0, _len = this.length; _i < _len; _i++) {
        o = this[_i];
    ...
    BreedSet.prototype.inRadius = function(o, d, meToo) {
      var a, d2, h, w, x, y;
      if (meToo == null) {
        meToo = false;
      }
    ...
    BreedSet.prototype.inCone = function(o, heading, cone, radius, meToo) {
      var a, h, rSet, w, x, y;
      if (meToo == null) {
        meToo = false;
      }
  ...
  ABM.Agent = (function() {
    ...
    Agent.prototype.fractionOfColor = function(color, fraction) {
      ...
      return u.fractionOfColor(color, fraction);
    };
    ...
    Agent.prototype.neighbors = function() {
      var agent, array, options, patch, _i, _j, _len, _len1, _ref, _ref1;
      options = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      array = this.breed.asSet([]);
      if (this.patch) {
        _ref = this.patch.neighbors;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          patch = _ref[_i];
          _ref1 = patch.agents;
    ...
    Agent.prototype.inCone = function(aset, cone, radius, meToo) {
      ...
      return aset.inCone(this.patch, this.heading, cone, radius, meToo);
    };
  ...
  ABM.Link = (function() {
    ...
    Link.prototype.draw = function(ctx) {
      ...
      ctx.save();
      ctx.strokeStyle = u.colorString(this.color);
  ...
  ABM.Model = (function() {
    ...
    function Model(divOrOptions, size, minX, maxX, minY, maxY, isTorus, hasNeighbors, isHeadless) {
      ...
      if (!isHeadless) {
        ...
        for (k in _ref) {
          ...
          v = _ref[k];
          this.contexts[k] = context = u.createLayer(this.div, this.world.pxWidth, this.world.pxHeight, v.z, v.ctx);
          if (context.canvas != null) {
            this.setCtxTransform(context);
          }
          if (context.canvas != null) {
    ...
    Model.prototype.setWorld = function(options) {
      var defaults, hasNeighbors, isHeadless, isTorus, key, maxX, maxXcor, maxY, maxYcor, minX, minXcor, minY, minYcor, numX, numY, pxHeight, pxWidth, size, value;
      defaults = {
        size: 13,
        minX: -16,
        maxX: 16,
      ...
      size = options.size, minX = options.minX, maxX = options.maxX, minY = options.minY, maxY = options.maxY, isTorus = options.isTorus, hasNeighbors = options.hasNeighbors, isHeadless = options.isHeadless;
      numX = maxX - minX + 1;
      numY = maxY - minY + 1;
      pxWidth = numX * size;
      pxHeight = numY * size;
  ...
  ABM.Patch = (function() {
    ...
    Patch.prototype.neighbors = null;

    Patch.prototype.neighbors4 = null;
    ...
    Patch.prototype.draw = function(ctx) {
      var x, y, _ref;
      ctx.fillStyle = u.colorString(this.color);
  ...
  ABM.Patches = (function(_super) {
    ...
    Patches.prototype.setNeighbors = function() {
      var four, neighbor, patch, _i, _j, _len, _len1, _ref, _results;
      _results = [];
      for (_i = 0, _len = this.length; _i < _len; _i++) {
        patch = this[_i];
        patch.neighbors = this.patchRectangle(patch, 1, 1);
        four = [];
        _ref = patch.neighbors;
        for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
          neighbor = _ref[_j];
          if (neighbor.x === patch.x || neighbor.y === patch.y) {
            four.push(neighbor);
          }
        }
        _results.push(patch.neighbors4 = this.asSet(four));
      }
      return _results;
    };
    ...
    Patches.prototype.patchRectangle = function(patch, dx, dy, meToo) {
      var nextPatch, rect, x, y, _i, _j, _ref, _ref1, _ref2, _ref3;
      if (meToo == null) {
        meToo = false;
      }
    ...
    Patches.prototype.floodFill = function(aset, fCandidate, fJoin, fCallback, fNeighbors, asetLast) {
      if (fNeighbors == null) {
      ...
      return Patches.__super__.floodFill.call(this, aset, fCandidate, fJoin, fCallback, fNeighbors, asetLast);
    };
